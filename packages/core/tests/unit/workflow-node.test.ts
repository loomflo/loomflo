...
describe('WorkflowNode', () => {
  ...
  describe('getValidTransitions', () => {
    it('returns correct transitions for each state', () => {
      expect(
        new WorkflowNode(makeNodeData({ status: 'pending' })).getValidTransitions(),
      ).toEqual(['waiting']);

      expect(
        new WorkflowNode(makeNodeData({ status: 'waiting' })).getValidTransitions(),
      ).toEqual(['running']);

      expect(
        new WorkflowNode(makeNodeData({ status: 'running' })).getValidTransitions(),
      ).toEqual(['review', 'done', 'failed', 'blocked']);

      expect(
        new WorkflowNode(makeNodeData({ status: 'review' })).getValidTransitions(),
      ).toEqual(['done', 'running', 'blocked', 'failed']);

      expect(
        new WorkflowNode(makeNodeData({ status: 'done' })).getValidTransitions(),
      ).toEqual([]);

      expect(
        new WorkflowNode(makeNodeData({ status: 'failed' })).getValidTransitions(),
      ).toEqual([]);

      expect(
        new WorkflowNode(makeNodeData({ status: 'blocked' })).getValidTransitions(),
      ).toEqual([]);
    });
  });
  ...
  describe('transition', () => {
    ...
    it('throws on invalid transition', () => {
      const node = new WorkflowNode(makeNodeData({ status: 'pending' }));
      expect(() => node.transition('done')).toThrow(
  ...
  describe('addAgent', () => {
    ...
    it('throws on duplicate agent ID', () => {
      ...
      expect(() => node.addAgent(makeAgent())).toThrow(
        'Agent "looma-1" already exists',
      );
    });
  });
  ...
  describe('updateAgent', () => {
    ...
    it('throws if agent not found', () => {
      const node = new WorkflowNode(makeNodeData());
      expect(() => node.updateAgent('missing', {})).toThrow(
  ...
  describe('removeAgent', () => {
    ...
    it('throws if agent not found', () => {
      const node = new WorkflowNode(makeNodeData());
      expect(() => node.removeAgent('missing')).toThrow(
        'Agent "missing" not found',
      );
    });
  });
