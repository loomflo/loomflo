...
describe('unregisterAgent', () => {
  it('removes the queue', async () => {
    ...
    await expect(
      bus.send('agent-2', 'agent-1', 'node-1', 'gone'),
    ).rejects.toThrow('recipient "agent-1" is not registered');
  });
  ...
  it('cleans up empty node entries', async () => {
    ...
    await expect(
      bus.send('anyone', 'anyone', 'node-1', 'msg'),
    ).rejects.toThrow('no agents registered for node "node-1"');
  });
});
...
describe('send', () => {
  ...
  it('rejects if no agents registered for node', async () => {
    await expect(
      bus.send('a', 'b', 'nonexistent', 'msg'),
    ).rejects.toThrow('no agents registered for node "nonexistent"');
  });
  ...
  it('rejects if sender not registered to node', async () => {
    ...
    await expect(
      bus.send('unknown-sender', 'receiver', 'node-1', 'msg'),
    ).rejects.toThrow('sender "unknown-sender" is not registered to node "node-1"');
  });
  ...
  it('rejects if recipient not registered to node', async () => {
    ...
    await expect(
      bus.send('sender', 'unknown-receiver', 'node-1', 'msg'),
    ).rejects.toThrow('recipient "unknown-receiver" is not registered to node "node-1"');
  });
...
describe('broadcast', () => {
  ...
  it('rejects if sender not registered', async () => {
    ...
    await expect(
      bus.broadcast('unknown', 'node-1', 'msg'),
    ).rejects.toThrow('sender "unknown" is not registered to node "node-1"');
  });
  ...
  it('rejects if no agents registered for node', async () => {
    await expect(
      bus.broadcast('a', 'nonexistent', 'msg'),
    ).rejects.toThrow('no agents registered for node "nonexistent"');
  });
});
...
describe('cross-node rejection', () => {
  it('agents in different nodes cannot communicate via send', async () => {
    ...
    await expect(
      bus.send('a', 'b', 'node-1', 'cross'),
    ).rejects.toThrow('recipient "b" is not registered to node "node-1"');

    // Sender on node-2, recipient on node-1 — recipient not found on node-2.
    await expect(
      bus.send('b', 'a', 'node-2', 'cross'),
    ).rejects.toThrow('recipient "a" is not registered to node "node-2"');
  });
