...
describe('SpecEngine', () => {
  ...
  beforeEach(async () => {
    projectPath = join(tmpdir(), `loomflo-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('estimateNodeCost', () => {
    ...
    it('should fall back to 1 task for plain text', () => {
      const node = createGraph(
        { a: { instructions: 'Do something simple' } },
        [],
      ).nodes['a']!;
    ...
    it('should scale cost with number of tasks', () => {
      const node1 = createGraph(
        { a: { instructions: '1. One' } },
        [],
      ).nodes['a']!;
      const node3 = createGraph(
        { a: { instructions: '1. One\n2. Two\n3. Three' } },
        [],
      ).nodes['a']!;
  ...
  describe('graph building', () => {
    ...
    it('should reject LLM output with invalid dependency references', async () => {
      const badRefGraph = JSON.stringify({
        nodes: [
          { id: 'node-1', title: 'A', instructions: 'Do A', dependencies: ['nonexistent'] },
        ],
      });
