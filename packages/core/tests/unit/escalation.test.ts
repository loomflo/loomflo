...
function makeMockSharedMemory(): { write: ReturnType<typeof vi.fn>; read: ReturnType<typeof vi.fn> } {
  return {
    ...
    read: vi.fn().mockResolvedValue({ content: '', name: '', path: '', lastModifiedBy: 'system', lastModifiedAt: new Date().toISOString() }),
  };
}
...
function makeConfig(overrides?: Partial<{
  ...
  const graphModifier = (overrides?.graphModifier ?? makeMockGraphModifier()) as GraphModifierLike & { calls: GraphModification[] };
...
describe('EscalationManager', () => {
  ...
  describe('error handling', () => {
    ...
    it('handles LLM response with JSON wrapped in text', async () => {
      const provider = makeMockProvider(
        makeLLMResponse('I think the best approach is {"action":"remove_node","nodeId":"node-auth","reason":"Redundant"} given the situation.'),
      );
      const cfg = makeConfig({ provider });
      const mgr = new EscalationManager(cfg);
