...
function makePassReview(): ReviewReport {
  return {
    verdict: "PASS",
    tasksVerified: [{ taskId: "looma-1", status: "pass", details: "All good" }],
...
function makeBlockedReview(): ReviewReport {
  return {
    verdict: "BLOCKED",
    tasksVerified: [{ taskId: "looma-1", status: "blocked", details: "Missing dependency" }],
...
function makeMockProvider(responses: LLMResponse[]): LLMProvider {
  ...
  return {
    complete: vi.fn(async (_params: CompletionParams): Promise<LLMResponse> => {
      ...
      if (!response) {
        throw new Error(
          `Unexpected LLM call #${String(callIndex + 1)} — no more responses configured`,
        );
      }
      callIndex++;
      return response;
...
function makeLoomiConfig(
  ...
  const provider = overrides?.provider ?? makeMockProvider([makeLLMResponse(defaultPlan)]);
...
describe("Loomi Retry Cycle", () => {
  ...
  describe("review FAIL → retry", () => {
    it("retries with adapted prompt when review returns FAIL", async () => {
      ...
      const provider = makeMockProvider([
        makeLLMResponse(
          makeTeamPlanResponse([{ id: "looma-1", task: "Implement auth", scope: ["src/auth/**"] }]),
        ),
        // Adapted plan for retry
        makeLLMResponse(
          makeTeamPlanResponse([
    ...
    it("escalates when review keeps returning FAIL after max retries", async () => {
      ...
      const responses = [
        ...
        makeLLMResponse(
          makeTeamPlanResponse([{ id: "looma-1", task: "Fix attempt 1", scope: ["src/auth/**"] }]),
        ),
        makeLLMResponse(
          makeTeamPlanResponse([{ id: "looma-1", task: "Fix attempt 2", scope: ["src/auth/**"] }]),
        ),
        makeLLMResponse(
          makeTeamPlanResponse([{ id: "looma-1", task: "Fix attempt 3", scope: ["src/auth/**"] }]),
        ),
      ];
  ...
  describe("multiple workers with mixed results", () => {
    it("only retries failed workers, not successful ones", async () => {
      ...
      const provider = makeMockProvider([makeLLMResponse(planResponse)]);
