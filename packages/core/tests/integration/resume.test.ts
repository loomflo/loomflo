...
function makeWorkflow(projectPath: string, nodeOverrides?: Partial<Record<string, Partial<Node>>>): Workflow {
...
describe('Workflow Resume (integration)', () => {
  ...
  it('should skip completed nodes and reset interrupted nodes', async () => {
    ...
    const workflow = makeWorkflow(projectPath, {
      'node-a': { status: 'done', startedAt: '2026-01-01T00:00:00.000Z', completedAt: '2026-01-01T01:00:00.000Z', cost: 0.5 },
      'node-b': { status: 'running', startedAt: '2026-01-01T01:00:00.000Z', retryCount: 1, cost: 0.3 },
    ...
    await appendEvent(projectPath, createEvent({
      type: 'node_completed',
  ...
  it('should reset nodes in review state', async () => {
    const workflow = makeWorkflow(projectPath, {
      'node-a': { status: 'done', startedAt: '2026-01-01T00:00:00.000Z', completedAt: '2026-01-01T01:00:00.000Z' },
