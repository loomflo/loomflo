...
vi.mock("../../../src/observation/api.js", () => ({
  DaemonHttpError: FakeDaemonHttpError,
  httpGet: vi.fn().mockResolvedValue([
    { id: "spec-01", title: "Define auth model", status: "done", cost: 0.12, agentCount: 1, retryCount: 0, startedAt: "2026-04-15T00:00:00Z", completedAt: "2026-04-15T00:00:42Z" },
...
describe("loomflo nodes", () => {
  ...
  it("without --all, filters out done and failed nodes", async () => {
    const writes: string[] = [];
    vi.spyOn(process.stdout, "write").mockImplementation((c) => {
      writes.push(typeof c === "string" ? c : c.toString());
      return true;
    ...
    const plain = stripAnsi(writes.join(""));
    // "done" node spec-01 should be filtered out
  ...
  it("returns empty state when the daemon reports no active workflow (404)", async () => {
    const writes: string[] = [];
    vi.spyOn(process.stdout, "write").mockImplementation((c) => {
      writes.push(typeof c === "string" ? c : c.toString());
      return true;
    ...
    (httpGet as unknown as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
      new FakeDaemonHttpError("/projects/proj_x/nodes", 404, "No active workflow"),
    );
    const { createNodesCommand } = await import("../../../src/commands/nodes.js");
    await createNodesCommand().parseAsync(["node", "nodes"]);
    const plain = stripAnsi(writes.join(""));
    expect(plain).toContain("No nodes to display");
