...
- [x] T157 [US-Quality] [P] Write unit tests for packages/sdk/src/client.ts (LoomFloClient: connect, disconnect, get, post, onEvent, chat — mock WebSocket and fetch) in packages/sdk/tests/unit/client.test.ts. Replace the trivial placeholder test.
- [x] T158 [US-Quality] After T154-T157: run `pnpm test -- -- --coverage` and verify packages/cli >= 50% lines and packages/sdk >= 60% lines. Add vitest.config.ts to cli with appropriate thresholds.

**Checkpoint**: CLI and SDK packages have meaningful test coverage. `pnpm test` CI green.

---

## Phase 14: Daemon Wiring & Dashboard Integration

**Purpose**: Fix the critical bug where `Daemon.start()` creates a bare Fastify server without registering any routes. Connect `createServer()` from `api/server.ts` into the daemon lifecycle so `/health`, `/nodes`, `/workflow`, `/events`, and all other API routes actually respond. Also wire up the dashboard static files so the SPA is served correctly.
...
## Phase 15: CLI UX & Config Fixes

**Purpose**: Full audit of the CLI revealed multiple broken UX flows. Once T159 fixes daemon routing, several commands will work — but some have design issues that need fixing independently.
...
## Phase 16: Launch Blockers — Resilience & Error Handling

**Purpose**: Audit revealed critical gaps that would cause silent failures or crashes in real usage. These must be fixed before any public launch.

### Launch Blocker 1 — No retry on Anthropic 429/529 (overload)
