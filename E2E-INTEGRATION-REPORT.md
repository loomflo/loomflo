...
> _Given_ the store is empty, _When_ the user runs `todo add "Buy groceries"`, _Then_ the system creates the store file if absent, persists a task with ID 1, title "Buy groceries", completed false, and a createdAt timestamp, and prints a confirmation message to stdout.
...
_Rapport généré par bertholt — 1er avril 2026, 08h00 UTC_
